...
async def process_document(
    ...
    지원되는 파일 형식:
    - PDF: 단일 및 다중 페이지 문서
    - 이미지: PNG, JPEG, JPG 등 일반 이미지 형식

    자동으로 파일 형식을 감지하여 적절한 OCR 처리를 수행합니다.
    한국어 텍스트 인식에 최적화되어 있으며, 높은 정확도를 제공합니다.
    """
    content_type = file.content_type or ""

    if content_type == 'application/pdf':
        return await process_pdf(file, merge_blocks, merge_threshold, create_sections, build_hierarchy_tree)
    elif content_type.startswith('image/'):
        from api.endpoints.process_image import process_image
        return await process_image(file, merge_blocks, merge_threshold, create_sections, build_hierarchy_tree)
    else:
        raise HTTPException(
            status_code=400,
            detail="지원되지 않는 파일 형식입니다. PDF, PNG, JPEG 파일만 지원됩니다."
        )